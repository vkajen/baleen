...
public class CoveringStructureHierarchy extends StructureHierarchy {
   ...
   */  
     ...
     private static Map<Annotation, Collection<Structure>> buildCovering(JCas jCas,
            Set<Class<? extends Structure>> structuralClasses)
    {
        Map<Annotation, Collection<Annotation>> covering;
        covering = JCasUtil.indexCovering(jCas, Annotation.class, Structure.class);

        Map<Annotation, Collection<Structure>> filteredMap = new HashMap<>();

        for (Annotation c : covering.keySet())
        {
            StructureUtil.filterAnnotations(covering.get(c), structuralClasses);
            filteredMap.put(c, StructureUtil.filterAnnotations(covering.get(c), structuralClasses));
        }

        return filteredMap;
//            Maps.transformValues(covering,
